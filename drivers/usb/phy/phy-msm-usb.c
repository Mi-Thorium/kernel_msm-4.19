...
static int mmi_msm_otg_get_ext_id_voltage(struct msm_otg *motg)
{
	...
	if (IS_ERR_OR_NULL(motg->mmi_usbin_chan)) {
		motg->mmi_usbin_chan = iio_channel_get(motg->phy.dev, "usbin");
		if (IS_ERR(motg->mmi_usbin_chan)) {
			pr_err("ext_id_voltage - iio_channel_get failed\n");
			return volts;
		}
	}

	//msm_otg_enable_ext_id(motg, 0);
	rc = iio_read_channel_processed(motg->mmi_usbin_chan, &result);
